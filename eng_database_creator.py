...
for wordform in wordform_counter:
    freq = 0
    art_freq = 0
    for pos in wordform_counter[wordform]:
        freq += wordform_counter[wordform][pos]
    if wordform in articular_counter:
        for pos in articular_counter[wordform]:
            art_freq += articular_counter[wordform][pos]
    wordform_stats[wordform] = [freq, art_freq, round(art_freq/freq, 4)]

for lemma in lemma_counter:
    freq = 0
    art_freq = 0
    for pos in lemma_counter[lemma]:
        freq += lemma_counter[lemma][pos]
    if lemma in articular_lemma_counter:
        for pos in articular_lemma_counter[lemma]:
            art_freq += articular_lemma_counter[lemma][pos]
    lemma_stats[lemma] = [freq, art_freq, round(art_freq/freq, 4)]

wordform_csv = pd.DataFrame.from_dict(wordform_stats, orient='index', columns=['Frequency', 'Articular Frequency',
                                                                               'Articular %'])
lemma_csv = pd.DataFrame.from_dict(lemma_stats, orient='index', columns=['Frequency', 'Articular Frequency',
                                                                         'Articular %'])
wordform_csv.to_csv('wordform_stats.csv')
lemma_csv.to_csv('lemma_stats.csv')
